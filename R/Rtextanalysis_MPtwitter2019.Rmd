...
To use this Notebook, just follow along and click on the green arrows. 

Install Packages (oackages only need to be installed once):
...
docs <- tm_map(docs, removeWords, c("http", "amp", "äôs", "ä", "donäôt")) 
# Remove punctuations
...
text_df2 %>%
  anti_join(stop_words) %>%
  count(word) %>%
  with(wordcloud(word,scale = c(6, 0.5), n, max.words = 100))
```
